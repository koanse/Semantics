...
namespace Cmp
{
    ...
    static public class Compare
    {
            ...
            foreach (SortedDictionary<Word, double> sd in arrSD)
            {
                ...
                foreach (Word w in arrK)
                {
                    ...
                    if (sdSyn.TryGetValue(w.norm, out arr))
                    {
                        ...
                        {
                            sd[new Word() { norm = syn }] += sd[w] * p;
                        }
                        else
                        {
                            Word wnew=new Word() { norm=syn};
                            sd.Add(wnew, sd[w] * p);
                        }
                        sd.Remove(w);
                    }
        ...
        */
        ...
        public static string[] arrPosRu = new string[] { 
            AttributeRu.сущ.ToString(), AttributeRu.глаг.ToString(), AttributeRu.прил.ToString(), AttributeRu.нар.ToString(),
            AttributeRu.числ.ToString(),AttributeRu.мест.ToString(),AttributeRu.прич.ToString(),AttributeRu.дееприч.ToString(),
            AttributeRu.междом.ToString(),AttributeRu.частица.ToString(),AttributeRu.союз.ToString(),AttributeRu.предлог.ToString(),
            AttributeRu.вводн_слово.ToString(),AttributeRu.предикатив.ToString(),AttributeRu.иностр.ToString(),AttributeRu.цифры.ToString(),
            AttributeRu.загл_буква.ToString(),AttributeRu.эвристика.ToString()
        };
            ...
            if (language == "ru")
            {
                ...
                while (reader.Read())
                {
                        ...
                        char.IsLetterOrDigit(reader.GetAttribute("Form")[0]))
                    {
                        ...
                        if((AttributeRu)pos==AttributeRu.инф)
                            pos=(int)AttributeRu.глаг;
                        list.Add(new Word()
                        {
        ...
        public static double KuznetsovCompare(SortedDictionary<Word, double>[] arrSDx, SortedDictionary<Word, double>[] arrSDy,
            ...
            out double hx, out double hy, out double hxy, out string rep)
        {
            ...
            SortedDictionary<Word, double>[] arrSDxy = new SortedDictionary<Word, double>[arr.Length];
            for (int i = 0; i < arrSDxy.Length; i++)
            {
                arrSDxy[i] = new SortedDictionary<Word, double>();
                ...
                for (int j = 0; j < arrKey.Length; j++)
                {
                    ...
                    if (arrSDy[i].TryGetValue(arrKey[j], out val))
                        arrSDxy[i].Add(arrKey[j], Math.Min(arrVal[j], val));
                }
            }
            ...
            for (int i = 0; i < arr.Length; i++)
            {
                ...
                double[] arrVal = arrSDy[i].Values.ToArray();
                for (int j = 0; j < arrKey.Length; j++)
                {
                    rep += string.Format("{0} - {1}<br>", arrKey[j].norm, arrVal[j]);
                }
            ...
            for (int i = 0; i < arr.Length; i++)
            {
                ...
                double[] arrVal = arrSDxy[i].Values.ToArray();
                for (int j = 0; j < arrKey.Length; j++)
                {
                    rep += string.Format("{0} - {1}<br>", arrKey[j].norm, arrVal[j]);
                }
    ...
    {
        public bool useI, useIW, useIW2, useCos;
        public SortedDictionary<string, double> sdW,sdWOpt;
        public Regression regI,regIW,regIW2,regCos;
        ...
        public void Load(string file)
        {
            ...
            regIW2=(Regression)bf.Deserialize(fs);
